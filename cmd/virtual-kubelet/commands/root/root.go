...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	if c.Provider != "" && c.Provider != "openstack" {
		return errdefs.InvalidInputf("provider %q is not supported: only the openstack is supported")
	}

	p, err := openstack.NewZunProvider(
